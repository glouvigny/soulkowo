SoulKowo = {
    ...
    contactList: {
        ...
        updateContact: function() {
            Persistent.set({'contacts': SoulKowo.contactList.contacts});
    ...
    connected: function() {
        ...
        if (SoulKowo.user.remember) {
            Persistent.set({'remember': true, 'login': SoulKowo.user.login, 'password': SoulKowo.user.password});
        } else {
            Persistent.set({'remember': false, 'login': 'exampl_e', 'password': 'p4s5w0rD'});
        }
    },
    ...
    notification: function(level, message) {
        ...
        } else if (Notifications && Notifications.send) {
            fct = Notifications.send;
        }
        if (fct != null) {
            fct(level, null, message);
        }
    },
    ...
    init: function() {
        ...
        Persistent.get(['remember', 'login', 'password', 'contacts'], function(data) {
